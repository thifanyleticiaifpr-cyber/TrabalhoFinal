...
document.getElementById("formularioDespesa").addEventListener("submit", function(event) { 
    ...
    var listadespesa = JSON.parse(localStorage.getItem('listagem')) || [] //pega a lisat de despesas do LocalSotarge (que é um array que guarda os objetos despesa, e permite salvar dados mesmo quando a gente fecha um naavegadpr), ou vai criar um array sem nada se não tiver nada salvo
    listadespesa.push(despesa) //adiciona despesa nova no array, push serve para add o novo item no FINAL (questão de prova0) do array
...
function salvar(){ //salvar o salario
    var salario = parseFloat(document.getElementById("salario").value).toFixed(2) //pega o valor do salario e trsnaforma ele em numero com casas decimais
...
function exibir() { //função exibe as despesas
    var listadepesa = JSON.parse(localStorage.getItem('listagem')) || [] //pega a lista de despesas do LocalStoage e se não tiver nada cria um array(que é um objeto que guarda varios intens) vazio
    ...
    for(let i=0; i<listadepesa.length; i++) { //laço para percorrer um array de despesas
        let li = document.createElement('li') //cria um li para cada despesa (linha da lista)
        li.textContent = 'Data: ' + listadepesa[i].data + ' Nome: ' + listadepesa[i].nome + ' Valor: R$' + listadepesa[i].valor //adiciona algo do array na linha da lista
...
function resumoFinanceiro(){ //função que exibe o resumo, ou seja, salario, despesas e saldo
    ...
    var salario = parseFloat(document.getElementById('salario').value).toFixed(2)//pega o salario do input e muda para numero com casas
    ...
    salariop.textContent = 'Salario: R$' + salario //adicioona o texto no paragrafo
    resumo.appendChild(salariop) //adiciona o paragrafo na div resumo que criei no html
 
    var listadepesa = JSON.parse(localStorage.getItem('listagem')) || [] 
    var listadepesa = 0 //variavel que vai armaenar o total de despesas
    for(let i = 0; i < listadepesa.length; i++){ //laço que vai passar por todas as despesas e somar elas
        totalDespesas += parseFloat(listadepesa[i].valor)//pega as despesas e soma em totalDespesas 
    }
    var despesasp = document.createElement('p') //cria paragrafo para mostrar as despesas (total)
    despesasp.textContent = 'Total de despesas: R$' + total_Despesas.toFixed(2)//adiciona o tecto no paragrafo novo
